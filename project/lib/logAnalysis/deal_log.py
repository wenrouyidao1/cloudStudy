...
def dealLog(log_path, cpu=85, memory=90, disk=90):
    logging.config.fileConfig('./lib/cnf/logging.conf')
...
# 处理得到的日志文件，进行邮件报警
def send_mail(log_path, user, password, cpu=85, memory=90, disk=90):
...
def insert_database(log_path, host, user, password, db, port=3306):
    ...
    with open(file=log_path, mode='r', encoding='utf8') as file:
        for lines in file.readlines():
            times = int(lines.split()[0].strip('"'))   # 需要去掉首尾多余字符，因为读出来的是字符串，带有多余的符号'"'
            hostname = lines.split('-')[1].strip().strip('"')
            data = json.loads(lines.split('-')[-1])
            for item in data.items():
                if item[0] == 'cpu':
